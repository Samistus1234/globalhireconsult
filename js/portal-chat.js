...
(function () {
  ...
  function renderThreadPane(peer, messages) {
    ...
    var head =
      '<div class="chat-thread-head">' +
        ...
        '</div>' +
        (activeThreadId ? '<button class="chat-email-btn" id="portal-chat-email-btn" title="Email ' + escapeHtml(peer.full_name) + '">📧 Email</button>' : '') +
      '</div>';
    ...
    var emailBtn = document.getElementById('portal-chat-email-btn');
  ...
  function ensureEmailModal() {
    var m = document.getElementById('portal-chat-email-modal');
    if (m) return;
    var d = document.createElement('div');
    d.id = 'portal-chat-email-modal';
    d.className = 'chat-modal-overlay';
    d.innerHTML =
      '<div class="chat-modal">' +
        '<div class="chat-modal-head"><div class="chat-modal-title">📧 Send email</div><button class="chat-modal-close" id="portal-chat-email-cancel">✕</button></div>' +
        '<div class="chat-modal-body" style="padding:var(--space-4) var(--space-5);overflow:auto;">' +
          '<div class="chat-email-to-label" id="portal-chat-email-to-label"></div>' +
          '<input class="chat-email-input" id="portal-chat-email-subject" placeholder="Subject" maxlength="150">' +
          '<textarea class="chat-email-input chat-email-body" id="portal-chat-email-body" placeholder="Write your message…" maxlength="5000"></textarea>' +
          '<div style="display:flex;justify-content:flex-end;gap:var(--space-2);">' +
            '<button class="chat-btn" id="portal-chat-email-cancel2">Cancel</button>' +
            '<button class="chat-btn chat-btn-primary" id="portal-chat-email-send">Send email</button>' +
          '</div>' +
        '</div>' +
      '</div>';
    document.body.appendChild(d);
    document.getElementById('portal-chat-email-cancel').addEventListener('click', closeEmailModal);
    document.getElementById('portal-chat-email-cancel2').addEventListener('click', closeEmailModal);
    document.getElementById('portal-chat-email-send').addEventListener('click', sendEmail);
  }
  function openEmailModal() {
    if (!activeThreadId) { alert('Open a conversation first'); return; }
    ensureEmailModal();
    var label = document.getElementById('portal-chat-email-to-label');
    if (label && currentThreadPeer) label.textContent = 'To: ' + currentThreadPeer.full_name;
    document.getElementById('portal-chat-email-modal').classList.add('open');
    var sub = document.getElementById('portal-chat-email-subject');
    if (sub) sub.focus();
  }
  function closeEmailModal() {
    ...
    var sub = document.getElementById('portal-chat-email-subject');
    var bod = document.getElementById('portal-chat-email-body');
    if (sub) sub.value = '';
    if (bod) bod.value = '';
  }
  async function sendEmail() {
    var subject = document.getElementById('portal-chat-email-subject').value.trim();
    var emBody = document.getElementById('portal-chat-email-body').value.trim();
    if (!subject || !emBody) { alert('Subject and message are required'); return; }
    var btn = document.getElementById('portal-chat-email-send');
