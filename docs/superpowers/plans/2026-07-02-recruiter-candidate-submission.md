...
### Task 1: Create `globalhire.recruiter_submitted_candidates` + docs table + RLS

> **Plan correction (2026-07-02):** the pre-existing `public.recruiter_submitted_candidates` table is NOT empty/latent — it belongs to a separate dormant referral/commission feature (live FKs both directions, a dependent view `recruiter_performance_metrics`, and a `has_role()`-based RLS policy). DO NOT touch, move, or repurpose it. Create a brand-new table instead.

**Files:**
- Create: `schema-v18-recruiter-submitted-candidates.sql` (migration, applied via Supabase MCP `apply_migration`)

**Interfaces:**
- Produces: `globalhire.recruiter_submitted_candidates` (base), `public.gh_recruiter_submitted_candidates` (view), `globalhire.recruiter_submission_documents` (base), `public.gh_recruiter_submission_documents` (view). Front-end uses `ghFrom('recruiter_submitted_candidates')`, `ghFrom('recruiter_submission_documents')`.
- Column set on `recruiter_submitted_candidates`: `id uuid pk default gen_random_uuid()`, `recruiter_id uuid not null` (the submitting recruiter = globalhire.profiles.id), `full_name text not null`, `email text`, `phone text`, `profession text`, `specialty text`, `experience_years int`, `current_country text`, `target_countries text[]`, `passport_number text`, `license_number text`, `profile_data jsonb`, `status text not null default 'new'`, `admin_status text not null default 'submitted' check (admin_status in ('submitted','under_review','shortlisted','placed','rejected'))`, `admin_note text`, `reviewed_by uuid`, `reviewed_at timestamptz`, `promoted_profile_id uuid`, `created_at timestamptz default now()`, `updated_at timestamptz default now()`.

- [ ] **Step 1: Confirm no name clash.** Verify `globalhire.recruiter_submitted_candidates` does not already exist (`execute_sql` against information_schema). Do NOT reference `public.recruiter_submitted_candidates` at all.
- [ ] **Step 2: Write migration** — `CREATE TABLE globalhire.recruiter_submitted_candidates (<the full column set above>);` then `CREATE TABLE globalhire.recruiter_submission_documents (id uuid pk default gen_random_uuid(), submission_id uuid not null references globalhire.recruiter_submitted_candidates(id) on delete cascade, recruiter_id uuid not null, doc_type text not null, file_name text, file_path text not null, mime_type text, file_size_bytes bigint, status text not null default 'pending', created_at timestamptz default now());`.
- [ ] **Step 3: Create `public.gh_recruiter_submitted_candidates` and `public.gh_recruiter_submission_documents` views** (`SELECT * FROM globalhire.<t>`), `GRANT SELECT, INSERT, UPDATE ON ... TO authenticated;` and matching grants on base tables, following the `gh_profiles` view definition as template.
...
- [ ] **Step 5: Apply** via Supabase MCP `apply_migration(project_id='evzhnsugmvtqgmvzwyix', name='v18_recruiter_submitted_candidates', query=<sql>)`.
- [ ] **Step 6: Verify** with `execute_sql`: `select count(*) from globalhire.recruiter_submitted_candidates;` (0), and `select * from public.gh_recruiter_submitted_candidates limit 1;` (no error). Confirm RLS: `get_advisors(type='security')` shows no new RLS-disabled warnings.
- [ ] **Step 7: Commit** `git add schema-v18-recruiter-clients.sql && git commit -m "feat(db): recruiter_submitted_candidates → globalhire schema + review columns + docs table + RLS"`.
...
**Interfaces:**
- Consumes: `window.ghSupabase`, `ghFrom('recruiter_submitted_candidates')`, `currentUser` (set in existing init).
...
- [ ] **Step 3:** Implement `async function loadMyCandidates()`: `var r = await ghFrom('recruiter_submitted_candidates').select('*').eq('recruiter_id', currentUser.id).order('created_at', {ascending:false});` render each as a card showing name, profession/specialty, `admin_status` badge, and `admin_note` if present (this is the feedback loop display). Empty state: "You haven't added any candidates yet."
...
**Interfaces:**
- Consumes: `ghFrom('recruiter_submitted_candidates')`, `currentUser`.
- Produces: inserts a `recruiter_submitted_candidates` row; returns new `id` used by Task 4 (document upload).
...
- [ ] **Step 2:** `submitCandidate()` validates required fields, then `var {data, error} = await ghFrom('recruiter_submitted_candidates').insert({recruiter_id: currentUser.id, full_name, email, phone, profession, specialty, experience_years: parseInt(...)||null, current_country, target_countries, passport_number, license_number, profile_data: {notes}, status:'new', admin_status:'submitted'}).select().single();` Keep `data.id` for the doc-upload step.
- [ ] **Step 3:** On success, open the document-upload step (Task 4) for `data.id`; on failure show an inline error.
- [ ] **Step 4: Verify:** add a candidate → row appears in `globalhire.recruiter_submitted_candidates` (check via `execute_sql`), card shows in the list with "Submitted" badge.
...
**Interfaces:**
- Consumes: `window.ghSupabase.storage`, `ghFrom('recruiter_submission_documents')`, the `recruiter_submitted_candidates.id` from Task 3.
- Produces: files in `gh-applicant-documents` + rows in `recruiter_submission_documents`.
...
- [ ] **Step 2:** For each file: `path = 'recruiter-clients/'+currentUser.id+'/'+clientId+'/'+docType+'-'+Date.now()+'.'+ext;` `await ghSupabase.storage.from('gh-applicant-documents').upload(path, file, {contentType:file.type});` then `await ghFrom('recruiter_submission_documents').insert({submission_id:clientId, recruiter_id:currentUser.id, doc_type:docType, file_name:file.name, file_path:path, mime_type:file.type, file_size_bytes:file.size, status:'pending'});`
- [ ] **Step 3:** Show uploaded docs on the candidate card; allow adding more later from the card.
- [ ] **Step 4: Verify:** upload a CV → object exists in bucket, row in `recruiter_submission_documents`.
...
**Interfaces:**
- Consumes: `ghFrom('recruiter_submitted_candidates')` (admin RLS returns all), `ghFrom('profiles')` for recruiter names.
- Produces: `loadSubmissions()` rendering a table with filters (recruiter, admin_status).

- [ ] **Step 1:** Load all submissions joined to recruiter name: `select *` from `gh_recruiter_submitted_candidates`, and a map of recruiter_id→full_name from `gh_profiles` where role='recruiter'. Render rows: candidate, profession/specialty, **source recruiter**, admin_status badge, created_at, "Review" action.
...
**Interfaces:**
- Consumes: `ghFrom('recruiter_submitted_candidates')`, `ghFrom('recruiter_submission_documents')`, storage signed URLs.
...
- [ ] **Step 3:** `updateSubmission(id, status, note)` → `ghFrom('recruiter_submitted_candidates').update({admin_status:status, admin_note:note, reviewed_by:currentUser.id, reviewed_at:new Date().toISOString()}).eq('id', id);` then invoke Task 7 notify.
...
- [ ] **Step 1:** "Promote to candidate pool" action: create a `globalhire.profiles` row (role 'applicant', source 'recruiter:{recruiter_id}'), copy docs to the candidate's document set, set `recruiter_submitted_candidates.promoted_profile_id`. 
...
- **Schema decision resolved:** T1 moves `recruiter_submitted_candidates` into `globalhire` + `gh_` view so `ghFrom()` works and RLS matches convention.
